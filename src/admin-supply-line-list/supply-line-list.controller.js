...
(function() {
    ...
    controller.$inject = [
        '$state', '$stateParams', 'supplyLines', 'supplyingFacilities', 'programs', 'TABLE_CONSTANTS'
    ];

    function controller($state, $stateParams, supplyLines, supplyingFacilities, programs, TABLE_CONSTANTS) {
        var vm = this;
        ...
        function getTableConfig() {
            return {
                caption: 'adminSupplyLineList.noSupplyLinesFound',
                displayCaption: !vm.supplyLines || vm.supplyLines.length === 0,
                columns: [
                    {
                        header: 'adminSupplyLineList.supplyingFacility',
                        propertyPath: 'supplyingFacility.name'
                    },
                    {
                        header: 'adminSupplyLineList.supervisoryNode',
                        propertyPath: 'supervisoryNode.code',
                        displayColumn: function(item) {
                            return !vm.showFacilityPopover(item);
                        }
                    },
                    {
                        header: 'adminSupplyLineList.supervisoryNode',
                        ...
                        popover: {
                            template: 'admin-supply-line-list/facilities-popover.html',
                            title: 'adminSupplyLineList.servedFacilities'
                        }
                    },
                    {
                        header: 'adminSupplyLineList.program',
                        propertyPath: 'program.name'
                    }
                ],
                actions: {
                    header: 'adminSupplyLineList.actions',
                    data: [
                        {
                            type: TABLE_CONSTANTS.actionTypes.REDIRECT,
                            text: 'adminSupplyLineList.view',
                            redirectLink: function(item) {
                                return 'openlmis.administration.supplyLines.view({id: \'' + item.id + '\'})';
                            }
                        }
                    ]
                },
