    ...
    function productGridCell($templateRequest, $compile, requisitionValidator, TEMPLATE_COLUMNS, COLUMN_TYPES,
        ...
        function link(scope, element) {
            ...
            function updateCellContents() {
                var templateUrl = '',
                    isQuarantined = Boolean(scope.lineItem.orderable.quarantined),
                    quarantinedColumns = TEMPLATE_COLUMNS.getColumnsToBeBlockedIfQuarantined(),
                    explanationColumn = quarantinedColumns[0];

                if (column.name === TEMPLATE_COLUMNS.SKIPPED) {
                    templateUrl = 'requisition-product-grid/product-grid-cell-skip.html';
                } else if (isQuarantined && quarantinedColumns.includes(column.name)) {
                    templateUrl = column.name === explanationColumn
                        ? 'requisition-product-grid/product-grid-quarantined-explanation.html'
                        : 'requisition-product-grid/product-grid-quarantined.html';
