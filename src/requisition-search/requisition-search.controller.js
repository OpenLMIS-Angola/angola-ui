...
(function() {
    ...
    angular.module('requisition-search').controller('RequisitionSearchController', RequisitionSearchController);
    RequisitionSearchController.$inject = ['$state', '$filter', '$stateParams', 'facilities', 'offlineService',
        'localStorageFactory', 'confirmService', 'requisitions', 'REQUISITION_STATUS', 'TABLE_CONSTANTS'];

    function RequisitionSearchController($state, $filter, $stateParams, facilities, offlineService,
                                         localStorageFactory, confirmService, requisitions, REQUISITION_STATUS,
                                         TABLE_CONSTANTS) {
        var vm = this,
         ...
         *
         * @description
         * Holds config for requisitions list table.
         */
        ...
        function getTableConfig() {
            return {
                caption: 'requisitionSearch.noRequisitionsFound',
                displayCaption: !vm.requisitions.length,
                columns: [
                    {
                        header: 'requisitionSearch.program',
                        propertyPath: 'program.name'
                    },
                    {
                        header: 'requisitionSearch.facility',
                    ...
                    {
                        header: 'requisitionSearch.period',
                        propertyPath: 'processingPeriod.name'
                    },
                    {
                        header: 'requisitionSearch.startDate',
                    ...
                    {
                        header: 'requisitionSearch.endDate',
                    ...
                    {
                        header: 'requisitionSearch.dateInitiated',
                    ...
                    {
                        header: 'requisitionSearch.status',
                        propertyPath: 'status',
                        template: function(item) {
                            return $filter('requisitionStatus')(item.status);
                        }
                    },
                    {
                        header: 'requisitionSearch.emergency',
                    ...
                    data: [
                        {
                            ...
                            text: 'requisitionSearch.viewRequisition',
                            classes: 'primary'
                        },
                        {
                            type: TABLE_CONSTANTS.actionTypes.CLICK,
                            onClick: function(item) {
                                vm.removeOfflineRequisition(item);
                            },
                            text: 'requisitionSearch.removeOfflineRequisition.label',
                            classes: 'danger',
                            displayAction: function(item) {
                                return item.$availableOffline;
                            }
                        }
                    ]
                },
